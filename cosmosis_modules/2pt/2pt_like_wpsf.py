...
class TwoPointLikelihood(GaussianLikelihood):
    ...
    def __init__(self, options):
        ...
        self.psf_template_file = options.get_string("psf_template_file")
    ...
    def extract_spectrum_prediction(self, block, spectrum):
        ...
        if self.do_xi_plus_psf and section=='shear_xi_plus':
            theory_vector = self.add_psf_terms(theory_vector,block,t='plus')
        if self.do_xi_minus_psf and section=='shear_xi_minus':
            theory_vector = self.add_psf_terms(theory_vector,block,t='minus')
    ...
    def add_psf_terms(self,theory, block, t=None):
        psf_template_file, alpha_beta, cal_section, nbin_combinations = self.psf_template_file, self.alpha_beta, self.cal_section, self.nbin_combinations

        if t=='plus':
            if not alpha_beta:
                psf_template = np.loadtxt(psf_template_file+'xi_plus.txt', unpack=True, usecols=[1])
            else:
        ...
        if t=='minus':
            if not alpha_beta:
                psf_template = np.loadtxt(psf_template_file+'xi_minus.txt', unpack=True, usecols=[1])
            else:
                pp = np.loadtxt(psf_template_file, unpack=True, usecols=1, skiprows=5)
                pq = np.loadtxt(psf_template_file, unpack=True, usecols=2, skiprows=5)
                qq = np.loadtxt(psf_template_file, unpack=True, usecols=3, skiprows=5)
