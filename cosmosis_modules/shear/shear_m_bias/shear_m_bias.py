...
def setup(options):
    ...
    # one global value
    m_per_bin = options.get_bool(option_section, "m_per_bin", default=True)
    input_m = options.get_string(option_section, "input_m", default="F")
    ...
    return m_per_bin, input_m, cl_section, cal_section, cross_section, cmbcross_section, verbose
...
def calibrate_section(block, section, m_a, m_b, input_m, verbose):
    if input_m != "F": 
        m_corrections = np.loadtxt(input_m)
    n_a = len(m_a)
    n_b = len(m_b)
    for i in range(n_a):
        for j in range(n_b):
...
def calibrate_shear_shear(block, section, cal_section, m_per_bin, input_m, verbose):
    ...
    calibrate_section(block, section, m, m, input_m, verbose)


def calibrate_position_shear(block, section, cal_section, m_per_bin, input_m, verbose):
    nbin_a, nbin_b = get_nbins(block, section)
...
def calibrate_shear_cmbkappa(block, section, cal_section, m_per_bin, input_m, verbose):
    nbin_a, nbin_b = get_nbins(block, section)
    if (nbin_b != 1):
...
def execute(block, config):
    m_per_bin, input_m, cl_section, cal_section, cross_section, cmbcross_section, verbose=config
    ...
    if do_auto:
        calibrate_shear_shear(
            block, cl_section, cal_section, m_per_bin, input_m, verbose)
    if do_cross:
        calibrate_position_shear(block, cross_section,
                                 cal_section, m_per_bin, input_m, verbose)
    if do_cmbcross:
        calibrate_shear_cmbkappa(block, cmbcross_section, cal_section, 
                                m_per_bin, input_m, verbose)
