...
def shear_catalog_formatter(data_hsc, output_path, delta_gamma=0.02):
    ...
    #Ixx = data_hsc['ishape_hsm_moments_11']
    #Iyy = data_hsc['ishape_hsm_moments_22']
    #Ixy = data_hsc['ishape_hsm_moments_12']
    #e = (Ixx - Iyy + 2j * Ixy) / (Ixx + Iyy + 2*np.sqrt(Ixx*Iyy-Ixy**2)) # Shear definition
    #e1 = e.real
    #e2 = e.imag
    _data['mcal_g1'] = data_hsc['ishape_hsm_regauss_e1'] # Distortion definition
    _data['mcal_g2'] = data_hsc['ishape_hsm_regauss_e2'] 
    _data['mcal_g1'] = 1/(1+m)*(e1/2*R-data_hsc['ishape_hsm_regauss_derived_shear_bias_c1']) # Corrected but with the distortion definition
    _data['mcal_g2'] = 1/(1+m)*(e2/2*R-data_hsc['ishape_hsm_regauss_derived_shear_bias_c2'])
